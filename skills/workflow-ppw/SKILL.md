---
name: workflow-ppw
description: PPW workflow detail layer for project inventory and process clarification after workflow selection is known. Use when the task is already confirmed as PPW, the user explicitly requests @PPW, or intake has classified the task as project inventory, asset mapping, current-state clarification, or process discovery. Produces dated and versioned workflow docs under the active project's root docs/YYYY_MM_DD_中文任务名_vN/ directory.
---

# Workflow PPW

Use this skill only when the workflow type is already known to be `PPW`.
...
- Keep global handoff policy in `role-based-agent-governance`; keep PPW stage rules here and in `references/ppw.md`.
